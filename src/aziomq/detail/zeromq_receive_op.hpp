...
class zeromq_raw_receive_op_base : public reactor_op
    AZIOMQ_ALSO_INERIT_TRACKED_OP {

public:
    zeromq_raw_receive_op_base(socket_ops::socket_type socket,
                               message & msg,
                               int flags,
                               func_type complete_func)
    ...
    static bool do_perform_receive(boost::asio::detail::reactor_op* base) {
        auto o = static_cast<zeromq_raw_receive_op_base*>(base);
        o->ec_ = boost::system::error_code();
        try {
            auto rc = socket_ops::receive(o->socket_, &o->msg_, o->flags_);
...
class zeromq_raw_receive_op : public zeromq_raw_receive_op_base {
public:
    BOOST_ASIO_DEFINE_HANDLER_PTR(zeromq_raw_receive_op);

    zeromq_raw_receive_op(socket_ops::socket_type socket,
                          message & msg,
                          Handler handler,
                          int flags)
        : zeromq_raw_receive_op_base(socket, msg, flags, &zeromq_raw_receive_op::do_complete)
        , handler_(std::move(handler))
    { }
    ...
    static void do_complete(boost::asio::detail::io_service_impl* owner,
                            ...
                            const boost::system::error_code&,
                            size_t) {
        auto o = static_cast<zeromq_raw_receive_op*>(base);
        AZIOMQ_TRACKED_OP_ON_COMPLETE(*o, o->ec_, o->bytes_transferred_);
